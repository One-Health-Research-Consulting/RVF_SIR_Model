...
muAE <- 0.0008378592                         # Aedes egg mortality rate - identified by Latin hypercube ~ 3.3 years
NAEmax <- 223880.6                           # Aedes carrying capacity - identified by Latin hypercube 
...
biteA <- 0.07108191                          # Aedes bite rate- identified by Latin hypercube 
...
q <- 0.7444736                               # transovarial transmission - identified by Latin hypercube 
...
NCEmax <-  110013.6                          # Culex carrying capacity - identified by Latin hypercube 
Tcsl <-  .71                                 # transmission  to Culex from sheep/lamb
Tslc <-  .31                                 # transmission to sheep/lamb from Culex
biteC <- 0.04952358                          # Culex bite rate - identified by Latin hypercube 
...
if(SA == TRUE){
  if(Var_Select == FALSE){
    ...
    params.matrix_trans <- cbind(DaysCumRainA, DaysCumRainC, TempAC, MinRainA, MinRainC,  DaysCanHatchA, CulexHatchDelay,
                                 ...
                                 EgAE, EgNAE, 
                                 muA, 
                                 NAEmax = lhs[,1] * (NAEmax.max - NAEmax.min) + NAEmax.min, 
                                 phiA, bhA, 
                                 epsilon, 
                                 waitA,
                                 biteA = lhs[, 2] * (biteA.max - biteA.min) + biteA.min,
                                 Tasl, 
                                 Tsla, 
                                 q = lhs[, 3] * (q.max - q.min) + q.min,
                                 alphaAE,
                                 muAE = lhs[, 4] * (muAE.max - muAE.min) + muAE.min,
                                 EgCE, EgNCE, delta, psiC, phiC, bhC, 
                                 muC, 
                                 muCE, 
                                 NCEmax = lhs[, 5] * (NCEmax.max - NCEmax.min) + NCEmax.min, 
                                 waitC,
                                 Tcsl, 
                                 Tslc, 
                                 biteC = lhs[, 6] * (biteC.max - biteC.min) + biteC.min) 
  }
}
